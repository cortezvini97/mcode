...
extern void executeCommand(const char* command);

void validate_args(int argc){
    if (argc < 2) {
        printf("mcode v1.0.0\n");
        printf("mcode --version\n");
...
int generateBinaryExecutableFile(int argc, char const *argv[]) {
    if (argc < 4) {
        printf("Uso: mcode --applyModdify <file.binary> <file_output>\n");
        return 0;
    }
...
int executeBinary(int argc, char const *argv[]) {
    ...
    // Obter o diretório temporário do Windows
    ...
    snprintf(output_filename, sizeof(output_filename), "%s%s_%d.exe", temp_path, "temp_binary", random_number);
    ...
    snprintf(command, sizeof(command), "\"%s\" %s", output_filename, "");
